...
/**
 * 이메일 → Firebase 키 (base64url) — emailVerifications/userEmails/emailIndex가 공유.
 * 이 앱은 goedu.kr(구글 워크스페이스) 이메일만 받으므로, 구글 메일함의 별칭 규칙
 * (local+태그@도메인은 무시, local 안의 점(.)도 무시 — 전부 같은 받은편지함으로 배달됨)을
 * 그대로 반영해 정규화한다. 이걸 안 하면 teacher+1@goedu.kr, teacher+2@goedu.kr,
 * te.acher@goedu.kr가 전부 "다른 이메일"로 취급돼 같은 구글 계정으로 이메일 인증만
 * 반복하면 emailIndex 중복 검사를 무한히 우회해 계정을 계속 새로 만들 수 있었다
 * (특히 0000 학번은 학번 중복검사·사다코인 결제 절차도 없어 사실상 무제한 매크로 가입 가능했음).
 */
...
const COMMON_STAGES = [
  ...
  { level:6,  name:'원시행성 원반',        cost:{type:'hydrogen',amount:10000},  successRate:0.81, sellPrice:15000,   protectionCost:1, drop:{key:ITEM_KEYS.STELLAR_WIND,    min:0,max:2} },
  { level:7,  name:'허빅-하로 천체',       cost:{type:'hydrogen',amount:15000},  successRate:0.75, sellPrice:25000,   protectionCost:1, drop:{key:ITEM_KEYS.STELLAR_WIND,    min:0,max:2} },
  { level:8,  name:'전주계열성',           cost:{type:'hydrogen',amount:22000},  successRate:0.70, sellPrice:50000,   protectionCost:1, drop:{key:ITEM_KEYS.STELLAR_WIND,    min:0,max:2} },
  { level:9,  name:'FU 오리온 폭발',       cost:{type:'hydrogen',amount:30000},  successRate:0.66, sellPrice:90000,   protectionCost:1, drop:{key:ITEM_KEYS.HYPERGIANT_CORE, min:0,max:2} },
  { level:10, name:'영년 주계열',          cost:{type:'hydrogen',amount:30000},  successRate:0.62, sellPrice:180000,  protectionCost:1, drop:{key:ITEM_KEYS.SUPERNOVA_GLOW,  min:0,max:2} },
  { level:11, name:'주계열성',             cost:{type:'hydrogen',amount:51000},  successRate:0.61, sellPrice:500000,  protectionCost:1, drop:{key:ITEM_KEYS.NEUTRON_CRUST,   min:0,max:2} },
  { level:12, name:'주계열 성숙기',        cost:{type:'hydrogen',amount:70000},  successRate:0.54, sellPrice:1000000, protectionCost:1, drop:{key:ITEM_KEYS.NEUTRON_CRUST,   min:0,max:2} },
  { level:13, name:'중심 수소 고갈',       cost:{type:'hydrogen',amount:80000},  successRate:0.50, sellPrice:2000000, protectionCost:2, drop:{key:ITEM_KEYS.PULSAR_SIGNAL,   min:0,max:2} },
];
...
const TRACKS = {
  ...
  track1: [
    { level:14, name:'적색왜성',        cost:{type:'hydrogen',amount:100000}, successRate:0.49, sellPrice:5000000,   protectionCost:3,  drop:{key:ITEM_KEYS.MAGNETAR_FLARE, min:0,max:2} },
    { level:15, name:'플레어별',        cost:{type:'hydrogen',amount:130000}, successRate:0.46, sellPrice:10000000,  protectionCost:4,  drop:{key:ITEM_KEYS.HAWKING,        min:0,max:2} },
    { level:16, name:'초장수명 주계열', cost:{type:'hydrogen',amount:170000}, successRate:0.44, sellPrice:20000000,  protectionCost:7,  drop:{key:ITEM_KEYS.DARK_MATTER,     min:0,max:2} },
  ...
  track2: [
    { level:14, name:'준거성',          cost:{type:'hydrogen',amount:100000}, successRate:0.49, sellPrice:5000000,   protectionCost:3,  drop:{key:ITEM_KEYS.MAGNETAR_FLARE, min:0,max:2} },
    { level:15, name:'적색거성',        cost:{type:'hydrogen',amount:130000}, successRate:0.46, sellPrice:10000000,  protectionCost:4,  drop:{key:ITEM_KEYS.HAWKING,        min:0,max:2} },
    { level:16, name:'헬륨 섬광',       cost:{type:'hydrogen',amount:170000}, successRate:0.44, sellPrice:20000000,  protectionCost:7,  drop:{key:ITEM_KEYS.DARK_MATTER,     min:0,max:2} },
  ...
  track3: [
    { level:14, name:'준거성',          cost:{type:'hydrogen',amount:100000}, successRate:0.49, sellPrice:5000000,   protectionCost:3,  drop:{key:ITEM_KEYS.MAGNETAR_FLARE, min:0,max:2} },
    { level:15, name:'청색초거성',      cost:{type:'hydrogen',amount:130000}, successRate:0.46, sellPrice:10000000,  protectionCost:4,  drop:{key:ITEM_KEYS.HAWKING,        min:0,max:2} },
    { level:16, name:'적색초거성',      cost:{type:'hydrogen',amount:170000}, successRate:0.44, sellPrice:20000000,  protectionCost:7,  drop:{key:ITEM_KEYS.DARK_MATTER,     min:0,max:2} },
  ...
  track4: [
    { level:14, name:'준거성',          cost:{type:'hydrogen',amount:100000}, successRate:0.49, sellPrice:5000000,   protectionCost:3,  drop:{key:ITEM_KEYS.MAGNETAR_FLARE, min:0,max:2} },
    { level:15, name:'O형 초거성',      cost:{type:'hydrogen',amount:130000}, successRate:0.46, sellPrice:10000000,  protectionCost:4,  drop:{key:ITEM_KEYS.HAWKING,        min:0,max:2} },
    { level:16, name:'밝은청색변광성',  cost:{type:'hydrogen',amount:170000}, successRate:0.44, sellPrice:20000000,  protectionCost:7,  drop:{key:ITEM_KEYS.DARK_MATTER,     min:0,max:2} },
  ...
  track5: [
    { level:14, name:'준거성',              cost:{type:'hydrogen',amount:100000}, successRate:0.49, sellPrice:5000000,   protectionCost:3,  drop:{key:ITEM_KEYS.MAGNETAR_FLARE, min:0,max:2} },
    { level:15, name:'극초거성',            cost:{type:'hydrogen',amount:130000}, successRate:0.46, sellPrice:10000000,  protectionCost:4,  drop:{key:ITEM_KEYS.HAWKING,        min:0,max:2} },
    { level:16, name:'극대광도 별',         cost:{type:'hydrogen',amount:170000}, successRate:0.44, sellPrice:20000000,  protectionCost:7,  drop:{key:ITEM_KEYS.DARK_MATTER,     min:0,max:2} },
...
const SHOP_ITEMS = [
  { id:'warp_9',       type:'warp',       targetLevel:9,  price:800000,   amount:null, successRate:1.00 },
  { id:'warp_13',      type:'warp',       targetLevel:13, price:5000000,  amount:null, successRate:0.80 },
  { id:'warp_14',      type:'warp',       targetLevel:14, price:7500000,  amount:null, successRate:0.60 },
  { id:'warp_15',      type:'warp',       targetLevel:15, price:10000000, amount:null, successRate:0.40 },
...
// 사다코인(외부 결제)으로 사는 상점 — 수소 충전 / 특별상품(+19강·+21강 즉시 지급).
// 여기 amount/targetLevel/coinPrice가 실제 과금·지급의 유일한 근거(api/shop/[action].js).
// name은 사다코인 결제 요청(payment-request)의 title로도 그대로 쓰인다.
// star-game/config.js의 동명 배열은 표시용 desc/special만 추가로 들고 있고
// id·type·amount·targetLevel·coinPrice·name은 반드시 여기와 동일해야 한다.
const COIN_SHOP_ITEMS = [
  { id:'hydrogen_1m',  type:'hydrogen', amount:1000000,  targetLevel:null, coinPrice:250,  name:'수소 100만' },
  { id:'hydrogen_5m',  type:'hydrogen', amount:5000000,  targetLevel:null, coinPrice:1000, name:'수소 500만' },
  { id:'hydrogen_10m', type:'hydrogen', amount:10000000, targetLevel:null, coinPrice:1500, name:'수소 1000만' },
  { id:'star_19',      type:'star',     amount:null,     targetLevel:19,   coinPrice:2000, name:'+19강 별' },
  { id:'star_21',      type:'star',     amount:null,     targetLevel:21,   coinPrice:2500, name:'+21강 별' },
];
...
const BATTLE_DAILY_CAP  = 5;     // 동일 상대 하루 최대 배틀 횟수
