...
function apiKey() {
  const key = process.env.SADA_API_KEY;
  if (!key) throw new Error('SADA_API_KEY 환경변수가 설정되지 않았습니다.');
  return key;
}
...
async function createPaymentRequest(studentId, amount, title) {
  ...
  try {
    ...
    // apiKey()가 던지는 설정 오류 등
    return { ok: false, status: 500, error: '사다코인 연동 설정 오류입니다. 관리자에게 문의하세요.' };
  }
  ...
  return { ok: false, status: 502, error: (body && body.message) || '사다코인 결제 요청에 실패했습니다.' };
}
...
async function getPaymentRequest(requestId) {
  ...
  return { ok: false, status: 502, error: (body && body.message) || '결제 상태 조회에 실패했습니다.' };
}
